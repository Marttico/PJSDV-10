...
Bed::Bed(int Port,string Prefix,CommandLineInput* CLI,  std::ofstream& FI):prefix(Prefix),cli(CLI),port(Port),wm(Port),ledMode(false), bestand(FI), ddd()
{
...
void Bed::behaviour()
{
    ...
    {
        zetLed(false);
        //bestand->writeLog("Bed::led automatisch uitgeschakeld");
    }
...
void Bed::triggerCommands()
{
    ...
    {
        //Put commands below.
        if(commandCompare(".ledaan"))
        {
            zetLed(true);
            bestand<<ddd<<"Bed::Led aan via command"<<endl;
            cli -> setExecuted();
        }
        if(commandCompare(".leduit"))
        {
            zetLed(false);
            bestand<<ddd<<"Bed::Led uit via command"<<endl;
            cli -> setExecuted();
        }
    }
}
