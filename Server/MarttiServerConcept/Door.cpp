...
Door::Door(int Port,int DoorOpenTimerDelay,string Prefix,CommandLineInput* CLI,  Column* colmn, std::ofstream& FI):
doorOpenTimerDelay(DoorOpenTimerDelay),doorAngle(70),prefix(Prefix),cli(CLI),port(Port),wm(Port),ledMode(false),inputButton(false),oldInputButton(inputButton),openPermissie(true),cl(colmn), bestand(FI)
{}
